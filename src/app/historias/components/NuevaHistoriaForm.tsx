...
import { HistoriaClinicaData, NacionalidadType, GeneroType, GrupoSanguineoType, EdoCivilType } from '@/types/historia'; // Ajusta la ruta
import { 
  OPCIONES_NACIONALIDAD, 
  OPCIONES_GENERO_HISTORIA, 
  OPCIONES_GRUPO_SANGUINEO,
  OPCIONES_EDO_CIVIL
} from '@/utils/constants'; // Ajusta la ruta
import FormGroupInput from '@/components/GuiaPaciente/FormGroupInput'; // Reutiliza o crea genéricos
import FormGroupSelect from '@/components/GuiaPaciente/FormGroupSelect';
import FormGroupTextarea from '@/components/GuiaPaciente/FormGroupTextarea'; // Necesitarás este
import { cn } from '@/utils/helpers';

interface NuevaHistoriaFormProps {
  initialData?: HistoriaClinicaData | null; // Para modo edición
  onSave: (data: HistoriaClinicaData) => Promise<void>; // Función para guardar
  onCancel: () => void;
}
...
const initialFormData: HistoriaClinicaData = {
  ...
  // Inicializa el resto de campos opcionales como string vacío o undefined
...
export default function NuevaHistoriaForm({ initialData, onSave, onCancel }: NuevaHistoriaFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    // Aquí podrías añadir validación más robusta
    // Si hay una foto seleccionada, la subirías primero y obtendrías la URL
    let dataToSave = { ...formData };
    if (selectedFoto) {
      // Lógica para subir la foto (ej. a Firebase Storage, S3, o tu backend)
      // const fotoUrl = await uploadImageFunction(selectedFoto); 
      // dataToSave.fotoUrl = fotoUrl; // Actualiza con la URL real
      console.log("Simulando subida de foto:", selectedFoto.name);
      dataToSave.fotoUrl = fotoPreview || ''; // Usa el preview mientras no haya subida real
    }
  ...
  return (
    <div className="p-4 sm:p-6 bg-bg-card-light dark:bg-bg-card-dark rounded-lg shadow-xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
          ...
          <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-5">
            ...
            <FormGroupInput label="Edad Cronológica:" name="edadCronologica" value={String(edadCronologica)} readOnly inputClassName={`${commonInputStyles} bg-gray-100 dark:bg-gray-800`} labelClassName={commonLabelStyles}/>
        ...
        <div className="pt-4 border-t border-border-light dark:border-border-dark">
          <button
            ...
            className="w-full flex justify-between items-center p-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-md text-text-light-base dark:text-text-dark-base font-medium transition-colors"
          >
        ...
        {showSeccionDetallada && (
          <div className="space-y-6 pt-6 border-t border-gray-200 dark:border-gray-700 mt-6">
            ...
            { (formData.genero === 'Femenino' || formData.genero === 'Femenino Deportivo') &&
                <FormGroupTextarea label="Antecedentes Gineco-Obstétricos:" name="antecedentesGinecoObstetricos" value={formData.antecedentesGinecoObstetricos || ''} onChange={handleChange} rows={3} textareaClassName={commonInputStyles} labelClassName={commonLabelStyles}/>
            }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
                ...
                {/* ... Añade todos los campos de Examen Funcional aquí ... */}
            ...
            <h3 className="text-lg font-semibold pt-4 text-primary dark:text-primary-light">Examen Físico</h3>
             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-4">
                ...
                <FormGroupInput label="IMC (kg/m²):" name="imc" type="number" value={formData.imc ?? ''} onChange={handleNumberChange} inputClassName={commonInputStyles} labelClassName={commonLabelStyles}/> {/* Podría ser calculado */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                ...
                {/* ... Añade todos los campos de Examen Físico (Cuello, Corazón, etc.) aquí como FormGroupTextarea o Input ... */}
            ...
            <FormGroupTextarea label="Impresión Diagnóstica:" name="impresionDiagnostica" value={formData.impresionDiagnostica || ''} onChange={handleChange} rows={4} textareaClassName={commonInputStyles} labelClassName={commonLabelStyles}/>
